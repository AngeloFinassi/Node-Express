...
const prodSchema = joi.object({
    ...
    categoria: joi.string().min(5).max(30).required(),
    preco: joi.number().min(0).max(100000).required(),
})

const prodUpdateSchema = joi.object({
    nome: joi.string().min(5).max(150),
    categoria: joi.string().min(5).max(30),
    preco: joi.number().min(0).max(100000),
})
